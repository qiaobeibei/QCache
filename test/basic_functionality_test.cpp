...
void testLruBasicFunctionality() {
    ...
    // 测试容量限制和LRU淘汰策略
    lru.put(4, "value4"); // 应该淘汰最久未使用的key=1
    assert(!lru.get(1, value)); // key=1应该被淘汰
    assert(lru.get(2, value) && value == "value2");
    assert(lru.get(3, value) && value == "value3");
    ...
    lru.put(5, "value5"); // 应该淘汰key=3（最久未使用）
    assert(lru.get(2, value) && value == "value2"); // key=2应该还在
    assert(!lru.get(3, value)); // key=3应该被淘汰
    assert(lru.get(4, value) && value == "value4");
...
void testSimplePerformanceComparison() {
    ...
    for (int alg = 0; alg < 3; ++alg) {
        for (int op = 0; op < OPERATIONS; ++op) {
            ...
            if (op % 3 == 0) { // 33%概率写入
                if (alg == 0) lru.put(key, value);
                else if (alg == 1) lfu.put(key, value);
                else arc.put(key, value);
            } else { // 67%概率读取
                std::string result;
                gets[alg]++;
                bool hit = false;
                if (alg == 0) hit = lru.get(key, result);
                else if (alg == 1) hit = lfu.get(key, result);
                else hit = arc.get(key, result);
